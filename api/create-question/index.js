...
module.exports = async function (context, req) {
  try {
    const tableClient = getTableClient("Questions");
    ...
    const existingQuestions = tableClient.listEntities({
      queryOptions: {
    ...
    for await (const entity of existingQuestions) {
      const number = parseInt(entity.rowKey.replace("Q", ""), 10);
    ...
    const questionId = `Q${String(maxNumber + 1).padStart(3, "0")}`;
